import type { Venture, Skill, Contact, Deal, AgentLog, Asset } from "@/types";
...
export const assets = {
  list: (ventureId: string) => factoryApi.get<Asset[]>(`/assets?venture_id=${ventureId}`),
  get: (id: string) => factoryApi.get<Asset>(`/assets/${id}`),
  bulkImport: (ventureId: string, assetData: Array<Record<string, unknown>>) =>
    factoryApi.post<{ imported: number }>("/assets/bulk", { ventureId, assets: assetData }),
  delete: (id: string) => factoryApi.delete<void>(`/assets/${id}`),
};
